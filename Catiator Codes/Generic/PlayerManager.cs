...
// Keep track of all the player stuff
...
public class PlayerManager : MonoBehaviour, IDamagable, IHealable, IMoveable
{
    ...
    // How long is player out after deing?
    ...
    public void UpdateHealthBar()
    {
        ...
        // Fill in healthbar with HealthAmount
    ...
    // Counting the amount of times the player died
    public int HasDied
    {
        get { return hasDied; }
        private set { hasDied++; }
    ...
    // Counting the amount of times the player killed
    ...
    // Remove health from player over time
    ...
    // when player killed a character
    public void KilledCharacter()
    {
        HasKilled = 1;

        // If player has killed enough characters to win
        if(HasKilled >= killsNeeded)
        {
            Debug.Log("Won");
            //Victory audio
